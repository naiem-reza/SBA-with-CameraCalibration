...
The basic photogrammetry principal geometry consists of three geometric entities: object space points (3D points),
corresponding image points (2D points), and perspective centers. Such a geometry can be formalized with collinearity equations.
...
To estimate the 6 EOPs, the image coordinates are first rectified using the calculated IOPs of the digital camera, consisting of the principal point (c_x,c_x) (in pixel), the focal length f (in pixel),
the coefficients of radial distortion (K_1, K_2,K_3) (in pixel) and the coefficients of decentring distortion (P1,P2) (pixel) and affinity and non-orthogonality (skew) coefficients (B1, B2) (in pixels).
According to equations, the image measurements (u, v) (pixel) are rectified to (u', v') (pixel) according to
...
# 1 -------------------- Import Data --------------------
EOP_init, IOP_init, TieXYZ, TieObs = Fun.Import_Data(dir_files)

# 2 -------------------- Preprocess --------------------
Observation_Tie, TieXYZ, EOP = Fun.preprocess(TieXYZ, TieObs, EOP_init)
GCP, Observation_gcp = Fun.Control_from_xml(dir_files, Control_start_id=TieXYZ[-1, 0] + 1)
Info_BA = Fun.Bundle_information(dir_files, GCP, TieXYZ, Observation_Tie, Observation_gcp, IOP_init, EOP)

# 7 -------------------- initial Weight Matrix --------------------
Weight = Fun.ComputeWeight(Info_BA, sigma_img, sigma_obj)

# 5 -------------------- Sparse Bundle Adjustment --------------------
BA = Fun.SBA_point_const(Weight, Info_BA, Max_iter, th=1e-9, Show=True)

Final_EOP = BA.Final_EOP
Final_IOP = BA.Final_IOP
Final_TieXYZ = BA.Coordinate_point
